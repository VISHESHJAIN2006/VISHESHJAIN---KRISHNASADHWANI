...
(() => {
  ...
  async function renderProblems() {
    ...
    const rows = problems.map((p) => `
      <div class="card">
        <div class="card-row">
          <div>
            <a href="#" data-open="${esc(p.slug)}" style="font-family: var(--display); font-weight:600; font-size:16px; text-decoration:none; color: var(--ink);">${esc(p.title)}</a>
            <div style="margin-top:6px; display:flex; gap:8px; align-items:center;">
              <span class="badge badge-${p.difficulty}">${p.difficulty}</span>
              ${p.topics.map((t) => `<span class="badge badge-NOT_ATTEMPTED">${esc(t)}</span>`).join("")}
            </div>
          </div>
          <span class="badge badge-${p.my_status}">${p.my_status.replace("_", " ")}</span>
        </div>
      </div>
    `).join("") || `<div class="empty-state">No problems published yet. Check back once your trainer publishes some.</div>`;
  ...
  async function renderProblem() {
    ...
    app.innerHTML = shell(`
      <button class="btn-link" id="back-btn" style="margin-bottom:14px;">&larr; back to problems</button>
      <div class="workspace">
          ...
          <div class="panel">
            <div class="editor-toolbar">
              ...
              <div class="btn-row" style="margin-top:0;">
                <button class="btn" id="run-btn">Run (sample tests)</button>
                <button class="btn btn-primary" id="submit-btn">Submit</button>
              </div>
            </div>
            <textarea class="code-editor" id="code-editor" spellcheck="false"></textarea>
            <div id="judge-status" class="muted" style="margin-top:8px; font-family: var(--mono); font-size:12px;"></div>
